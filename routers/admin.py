...
def _ensure_todays_trips(db: Session, target_date: date):
    ...
    existing = db.query(models.Trip).filter(models.Trip.trip_date == target_date).all()
    existing_keys = {
    ...
    for van in vans:
        for pw in pickup_windows:
            ...
            if key not in existing_keys:
                db.add(models.Trip(van_id=van.id, trip_date=target_date, trip_type="pickup", pickup_window_id=pw.id))
                created_any = True
        for dw in drop_windows:
            key = (van.id, "drop", None, dw.id)
            if key not in existing_keys:
                db.add(models.Trip(van_id=van.id, trip_date=target_date, trip_type="drop", drop_window_id=dw.id))
                created_any = True
    if created_any:
        db.commit()
...
def _apply_weekly_defaults(db: Session, target_date: date):
    ...
    for default in defaults:
        ...
        if student.request_date == target_date:
            continue  # already explicitly planned this exact date, don't override
...
def _assign_side(db: Session, target_date: date, trip_type: str):
    """
    Handles either the pickup side or the drop side of assignment.
    Only touches students who actually requested that side (their
    pickup_window_id or drop_window_id is set) and who don't already
    have a real trip on that side - safe to re-run any number of times.
    """
    ...
    students = db.query(models.Student).filter(
        ...
        models.Student.request_date == target_date,
        window_id_field.isnot(None),
    ...
    for window_id, areas_dict in by_window_area.items():
        trips_this_window = db.query(models.Trip).filter(
            models.Trip.trip_date == target_date,
        ...
        for area, area_students in areas_dict.items():
            ...
            for student in area_students:
                ...
                # van_id is set here on EITHER side now, not just pickup -
                # a drop-only student needs a van_id too, for the live map.
...
def run_assignment(
    ...
    _ensure_todays_trips(db, today)

    pickup_assigned = _assign_side(db, today, "pickup")
    db.commit()
    drop_assigned = _assign_side(db, today, "drop")
    db.commit()

    # A student is "assigned" once every side they actually asked for
    # has a real trip. A drop-only student only needs drop_trip_id, a
    # pickup-only student only needs pickup_trip_id, a student who asked
    # for both needs both.
    students = db.query(models.Student).filter(
        models.Student.status == "requested",
    ...
    fully_assigned = 0
    for s in students:
        wants_pickup = s.pickup_window_id is not None
        wants_drop = s.drop_window_id is not None
        pickup_ok = (not wants_pickup) or s.pickup_trip_id is not None
        drop_ok = (not wants_drop) or s.drop_trip_id is not None
        if (wants_pickup or wants_drop) and pickup_ok and drop_ok:
            s.status = "assigned"
            fully_assigned += 1
    db.commit()
